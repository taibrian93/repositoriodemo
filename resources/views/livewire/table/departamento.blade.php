<div>
    <x-data-table :data="$data" :model="$departamentos">
        <x-slot name="head">
            <tr>
        ...
        <x-slot name="body">
            @foreach ($departamentos as $departamento)
                <tr x-data="window.__controller.dataTableController({{ $departamento->id }})">
                    {{-- <td>{{ $departamento->id }}</td> --}}
                    <td>{{ $departamento->descripcion }}</td>
                    <td>{{ $departamento->codigoDepartamental }}</td>
                    {{-- <td>{{ $departamento->codigo }}</td> --}}
                    <td>{{ $departamento->created_at->format('d M Y H:i') }}</td>
                    <td class="whitespace-no-wrap row-action--icon">
                        <a role="button" href="/departamento/edit/{{ $departamento->id }}" class="mr-3"><i class="fa fa-16px fa-pen"></i></a>
